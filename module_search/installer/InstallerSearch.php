...
class InstallerSearch extends InstallerBase implements InstallerRemovableInterface
{
    ...
    public function install()
    {
        ...
        //register the module
        $this->registerModule('search', _search_module_id_, '', '', $this->objMetadata->getStrVersion(), false);
    ...
    public function update()
    {
        ...
        $arrModule = SystemModule::getPlainModuleData($this->objMetadata->getStrTitle(), false);
        if ($arrModule['module_version'] == '7.2') {
            $strReturn .= $this->update_72_721();
        }
    ...
    private function update_72_721()
    {
        $strReturn = "Updating to 7.2.1...\n";

        $strReturn .= 'Deregister backend controller' . PHP_EOL;

        $module = SystemModule::getModuleByName('search');
        $module->setStrNameAdmin('');
        $module->setIntNavigation(0);
        ServiceLifeCycleFactory::getLifeCycle($module)->update($module);

        $this->updateModuleVersion($this->objMetadata->getStrTitle(), '7.2.1');
        return $strReturn;
    }
