...
class GraphJqplot implements GraphInterfaceFronted
{
    ...
    private function preGraphGeneration()
    {
        ...
        $arrSeriesStackedBarCharts = $this->getSeriesObjectsByChartType([GraphJqplotCharttype::STACKEDBAR, GraphJqplotCharttype::STACKEDBAR_HORIZONTAL]);
        ...
        $arrSeriesStackedBarCharts = $this->getSeriesObjectsByChartType([GraphJqplotCharttype::STACKEDBAR]);
        $arrSeriesLineCharts = $this->getSeriesObjectsByChartType([GraphJqplotCharttype::LINE, GraphJqplotCharttype::LINE_Y2AXIS]);
        ...
        if ($this->containsChartType(GraphJqplotCharttype::PIE) && $this->arrXAxisTickLabels != null) {
            foreach ($this->arrSeriesData as $keySeries => $objSeriesData) {
                ...
                foreach ($this->arrXAxisTickLabels as $keyLabel => $strLabelData) {
                    $arrSeries[] = [$strLabelData, $arrDataPointsTemp[$keyLabel]->getFloatValue()];
                }
                $arrData[] = $arrSeries;
            }
        } else {
    ...
    public function setBitRenderLegend($bitRenderLegend)
    {
        if ($bitRenderLegend === true) {
            ...
            if ($this->containsChartType(GraphJqplotCharttype::PIE)) {
                $this->arrOptions["legend"]["renderer"] = "$.jqplot.EnhancedPieLegendRenderer";
            } else {
